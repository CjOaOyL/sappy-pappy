...
export const handler = async (event) => {
  ...
  try {
    ...
    const res = await mutateTransactions(list => {
      ...
      for (const t of list) {
        ...
        if (t.type !== 'expense' || !t.reimbursable) {
          return { write: false, status: 400, error: `Transaction ${t.id} is not a reimbursable expense` };
        }
        if (t.linkedReimbursementId) {
          return { write: false, status: 400, error: `Expense ${t.id} already reimbursed` };
        }
        coveredTotal += Number(t.amount) || 0;
        targets.push(t);
      }
      if (targets.length !== ids.length) {
        return { write: false, status: 404, error: 'Some expense ids not found' };
      }
    ...
    return { statusCode: 200, headers, body: JSON.stringify({ ok: true, reimbursement: res.reimbursement }) };
  } catch (err) {
